...
This fork adds a local pipeline dashboard on top of the upstream framework:
**Hunt** (region-scoped scrape) → **Recommend** (`/rank`) → **Tailor** (`/apply` docs) → **Apply** (auto-submit) → **Interview** → **Outcome**.
Next.js UI on :3000, FastAPI on :8000, everything in Docker. See [`dashboard/README.md`](dashboard/README.md).
